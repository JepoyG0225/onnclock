...
export async function PATCH(
  ...
  if (action === "reverse") {
    if (correction.status !== "APPROVED") {
      return NextResponse.json(
        { error: "Only approved corrections can be reversed" },
        { status: 400 },
      );
    }
    if (!HR_ROLES.includes(ctx.role)) {
      return NextResponse.json(
        { error: "Insufficient permissions to reverse" },
        { status: 403 },
      );
    }
  ...
  if (decision.usedWorkflow) {
    if (!decision.authorized) {
      return NextResponse.json(
        { error: "Not authorized for this approval level" },
        { status: 403 },
      );
    }
  } else if (!(
  ...
  const newTrail = [
    ...trailOf(correction.approvalTrail),
    decision.trailEntry,
    ...
    if (decision.plan) {
      await notifyAfterApprove({
        ...
        ctx: {
          ...
          requesterEmployeeId: correction.employeeId,
          requesterDepartmentId,
          link: "/time-corrections",
          vars: { requestType: "Time Correction", requesterName },
        },
