...
export default function ThirteenthMonthPage() {
  ...
  const [selectedEntry, setSelectedEntry] =
    useState<ThirteenthMonthEntry | null>(null);
    ...
    try {
      ...
      if (s) {
        const wrapsAcrossYear =
          ...
          (Number(s.startMonth) === Number(s.endMonth) &&
            Number(s.startDay) > Number(s.endDay));
        setCoverageStart(
          toDateInput(
            Number(s.startMonth),
            Number(s.startDay),
            wrapsAcrossYear ? year - 1 : year,
          ),
        );
  ...
  const totalAmount = entries.reduce(
    (sum, e) => sum + Number(e.thirteenthAmount),
    0,
  );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            PD 851 - Must be paid on or before December 24. Non-taxable up to
            PHP 90,000.
          </p>
          {coverageStart && coverageEnd && (
            <p className="text-xs text-gray-500 mt-1">
              Coverage:{" "}
              {format(new Date(`${coverageStart}T00:00:00`), "MMM d, yyyy")} to{" "}
              {format(new Date(`${coverageEnd}T00:00:00`), "MMM d, yyyy")}
            </p>
          )}
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button data-tour="tm-compute" onClick={compute} disabled={computing}>
            {computing ? (
      ...
      <div className="rounded-2xl border border-gray-200 border-l-4 border-l-[var(--brand-primary)] bg-white p-4 shadow-sm">
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  {selectedEntry.employee.employeeNo} -{" "}
                  {selectedEntry.employee.department?.name}
                </p>
              </div>
              <button
                onClick={() => setSelectedEntry(null)}
            ...
            <div className="grid grid-cols-6 gap-1.5 mb-4">
              {MONTHS.map((m, i) => {
                ...
                return (
                  <div
                    key={m}
                    className={`rounded-lg p-2 text-center ${val > 0 ? "border border-blue-200 bg-blue-50" : "border border-gray-200 bg-gray-50"}`}
                  >
                    <p className="text-xs text-gray-400">{m}</p>
                    <p
                      className={`text-xs font-bold ${val > 0 ? "text-[var(--brand-primary)]" : "text-gray-400"}`}
                    >
                      {val > 0 ? peso(val) : "-"}
                    </p>
                  </div>
                ...
                entries.map((entry) => (
                  <tr key={entry.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-right text-gray-700">
                      {Number(entry.proRatedMonths).toFixed(0)}/12
                    </td>
                    <td className="px-4 py-3 text-right font-medium text-gray-900">
                      {peso(Number(entry.totalBasicPaid))}
                    </td>
                    <td className="px-4 py-3 text-right font-bold text-[var(--brand-ink)]">
                      {peso(Number(entry.thirteenthAmount))}
                    </td>
                    <td className="px-4 py-3 text-center">
                      {entry.isPaid ? (
                        <Badge className="h-7 w-32 justify-center gap-1 border-emerald-200 bg-emerald-50 text-emerald-700">
                          <CheckCircle className="w-3 h-3" /> Paid{" "}
                          {entry.paidAt
                            ? format(new Date(entry.paidAt), "MMM d")
                            : ""}
                        </Badge>
                      ) : (
                        <Badge className="h-7 w-32 justify-center gap-1 border-blue-200 bg-blue-50 text-blue-700">
