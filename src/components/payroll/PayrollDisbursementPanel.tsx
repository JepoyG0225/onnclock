...
const POLL_MS = 8_000
const QR_LIFETIME_MS = 29 * 60 * 1000
...
function TopUpModal({
  ...
  function handleSuccess() {
    onConfirmed(confirmedAmount.current)
    onClose()
  }
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6 space-y-5">
        ...
        {phase === 'input' && (
          <>
            <p className="text-sm text-gray-500">
              Fund your disbursement wallet using GCash or Maya QR code.
            </p>
        ...
        {phase === 'qr' && (
          <>
            ...
            <div className="text-center space-y-1">
              ...
              <p className="text-xs text-gray-500">Scan with GCash · Maya · BPI · BDO · UnionBank</p>
            </div>
        ...
        {phase === 'success' && (
          <div className="flex flex-col items-center gap-3 py-6 text-center">
            ...
            <Button className="mt-2" onClick={handleSuccess}>Done</Button>
          </div>
        )}
        ...
        {phase === 'expired' && (
          <div className="flex flex-col items-center gap-3 py-6 text-center">
            ...
            <p className="text-sm text-gray-500">The QR code timed out. Please generate a new one.</p>
...
function SourceAccountModal({
  ...
  initial: { accountNo: string | null; accountName: string | null; bic: string | null }
  onClose: () => void
  onSaved: () => void
}) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold">Set Up Disbursement Source</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600"><X className="w-5 h-5" /></button>
        </div>
        <p className="text-sm text-gray-500">
          The source account is your company&apos;s bank or PayMongo wallet that funds are sent from.
          You must have Money Movement enabled on your PayMongo account.
        </p>
        <div className="space-y-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-gray-400 mt-0.5">8 or 11 characters. For PayMongo wallet: PAEYPHM2XXX</p>
          </div>
        </div>
...
export default function PayrollDisbursementPanel({ runId }: { runId: string }) {
    ...
    async function pollStatus() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (preview && !['PROCESSING', 'PENDING'].includes(data.status)) {
  ...
  async function initiateDisbursement() {
    ...
    if (!preview.sourceAccount.accountNo) { toast.error('Please configure your source account first.'); return }

    const confirmed = window.confirm(
      `Disburse ${fmtPHP(preview.summary.totalAmount)} to ${preview.summary.readyCount} employees?\n\n` +
      `InstaPay (≤₱50k): ${preview.summary.instapayCount} employees\n` +
      `PesoNet (>₱50k): ${preview.summary.pesonetCount} employees\n\n` +
      `Note: PesoNet transfers may take up to 3 banking days to complete.`,
    )
    if (!confirmed) return
  ...
  const { wallet, sourceAccount, summary, items, disbursement } = preview
        ...
        <CardContent className="space-y-5">
          ...
          <div className="flex gap-2 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
            ...
            <p className="text-xs text-amber-700">
              <span className="font-semibold">Processing time:</span> InstaPay transfers are real-time (up to 20 min).
              PesoNet transfers (amounts over ₱50,000) may take up to <span className="font-semibold">3 banking days</span> to reflect in the recipient&apos;s account.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="flex items-center justify-between rounded-xl border px-4 py-3 bg-gray-50">
              <div className="flex items-center gap-2">
                <Building2 className="w-4 h-4 text-[#2E4156]" />
                <div>
                  <p className="text-[11px] text-gray-500 uppercase tracking-wide">Source Account</p>
                  {sourceAccount.accountNo ? (
                    <>
                      <p className="text-sm font-medium">{sourceAccount.accountName}</p>
                      <p className="text-[11px] text-gray-500">{sourceAccount.accountNo} · {sourceAccount.bic}</p>
                    </>
                  ) : (
              ...
              <Button size="sm" variant="outline" onClick={() => setShowSource(true)}>
                {sourceAccount.accountNo ? 'Edit' : 'Set Up'}
              </Button>
            </div>
          </div>
          ...
          {!disbursement && (
            <div className="rounded-xl border bg-[#f8f9fb] p-4 space-y-3">
              <div className="flex flex-wrap gap-4 text-sm">
                <div>
                  <span className="text-gray-500">Total to disburse: </span>
                ...
                >
                  <div className="flex items-center justify-between bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-xs text-amber-700">
                        <span className="font-semibold">{summary.missingBankDetails} employee{summary.missingBankDetails > 1 ? 's' : ''}</span> missing bank account or BIC code — will be skipped.
                      </p>
                    </div>
                  ...
                  {missingItems.map(item => (
                    <div key={item.payslipId} className="text-xs text-gray-600 flex items-center gap-2">
                      ...
                      <span className="text-gray-400">
                        {!item.bankAccountNo ? 'no account number' : 'no BIC code'}
                      </span>
                    </div>
                  ))}
                  <p className="text-[11px] text-gray-400 mt-1">
                    Update these employees&apos; bank details in their profile to include them in disbursement.
                  </p>
                </div>
              )}
              ...
              return (
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    {referenceNo && (
                      <p className="text-[11px] text-gray-400">Ref: {referenceNo}</p>
                    )}
                  </div>
                  <div className="flex items-center gap-3 ml-3 shrink-0">
                    ...
                    {disbursement && statusBadge(displayStatus)}
                    {!disbursement && <BadgeCheck className="w-4 h-4 text-green-500" />}
                  </div>
                </div>
              )
            })}
          ...
          {/* Disbursement completed summary */}
            ...
            <div className="flex items-center gap-2 bg-green-50 border border-green-200 rounded-lg px-4 py-3">
              ...
              <div>
                ...
                <p className="text-xs text-green-700">
                  ...
                  {disbursement.completedAt && ` Completed ${new Date(disbursement.completedAt).toLocaleDateString()}.`}
                </p>
              </div>
            </div>
          )}
          ...
          {disbursement?.status === 'PARTIAL' && (
            <div className="flex items-center gap-2 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3">
              ...
              <div>
                ...
                <p className="text-xs text-amber-700">Some transfers failed. Contact PayMongo support for failed items.</p>
              </div>
            </div>
          )}
        </CardContent>
