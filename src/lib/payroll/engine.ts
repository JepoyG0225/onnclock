...
export function computePayroll(input: PayrollInput): PayrollResult {
  ...
  //
  // HOURLY / DAILY rate-type guard: when time-tracking is ON, regularHours
  // (and therefore basicPay) is computed from the ACTUAL clock-in time — if
  // an employee is 30 min late, regularHours is already 30 min shorter and
  // basic pay is already reduced proportionally, so a separate
  // minuteRate × lateMinutes deduction would double-count.
  //
  // BUT when time-tracking is OFF, basic pay is the FULL daily rate × days
  // (not hour-prorated), so the late shortfall is NOT already reflected —
  // the late deduction must apply so tardiness shows as its own line.
  const hourProrated = employee.trackTime !== false  // default true (legacy callers)
  const skipLate = period.disableLateDeductions
    || employee.disableLateDeduction === true
    || ((employee.rateType === 'HOURLY' || employee.rateType === 'DAILY') && hourProrated)
